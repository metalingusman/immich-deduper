...
class pth:
	...
	def full(path: Optional[str]) -> str:
		...
		for hostPth, contPth in pathHostMap.items():
			...
			if nor.startswith(hostPth + '/') or nor == hostPth: return nor.replace(hostPth, contPth.rstrip('/'), 1)
...
if len(dto.pathImmich) == 0 and envs.immichPath:
	...
	else: lg.warn(f'init immichPath: path not found [{envs.immichPath}]')
...
if len(dto.pathThumb) == 0 and envs.immichThumb:
	...
	else: lg.warn(f'init immichThumb: path not found [{envs.immichThumb}] - check volume mount')
